...
import { fechaMostrarFromIso, diferenciaT } from "../../services/Fechas";
import { venciMFechaClaseCelda } from "../../services/funcVenc";
...
const VencimientosList = (props) => {
  ...
  const retrieveClientes = useCallback(async () => {
    try {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: "Fecha1",
        accessor: "vencimiento1",
        Cell: ({ value }) => {
          const color = venciMFechaClaseCelda(diferenciaT(value, "d"));
          return <div className={color}>{fechaMostrarFromIso(value)}</div>;
        },
      },
      {
        Header: "Fecha2",
        accessor: "vencimiento2",
        Cell: ({ value }) => {
          const color = venciMFechaClaseCelda(diferenciaT(value, "d"));
          return <div className={color}>{fechaMostrarFromIso(value)}</div>;
        },
  ...
  return (
    <div>
      <div className="alert alert-primary content-center" role="alert">
